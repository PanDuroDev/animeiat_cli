...
PROVIDER_ID = ProviderId.WITANIME
...
class WitAnimeProvider:
    ...
    async def fetch_episodes(self, url: str) -> list[dict]:
        eps, err = await _fetch_episodes_list_httpx(url, ProviderId.WITANIME)
        if eps is not None:
            return eps
        eps, err = await fetch_episodes_list_async(url, ProviderId.WITANIME)
        return eps or []

    async def fetch_episodes_with_cookies(self, url: str, cookies: list) -> list[dict]:
        eps, err = await _fetch_episodes_list_httpx(url, ProviderId.WITANIME, cookies)
        if eps is not None:
            return eps
        eps, err = await fetch_episodes_list_async(url, ProviderId.WITANIME, cookies)
        return eps or []

    async def _resolve(self, episode_url: str, cookies=None):
        stream = await _scrape_one_stream_httpx({"episode": 0, "page_url": episode_url}, ProviderId.WITANIME, cookies)
        if stream:
            return {"url": stream, "quality": _classify_stream_quality(stream)}
        stream = await _scrape_one_stream_playwright({"episode": 0, "page_url": episode_url}, ProviderId.WITANIME, cookies)
        if stream:
            return {"url": stream, "quality": _classify_stream_quality(stream)}
        return None

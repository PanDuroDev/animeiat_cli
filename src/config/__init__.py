"""
Configuration management for animeiat-cli.
...
def get_config_path_pointer():
    return os.path.join(os.path.expanduser("~"), ".animeiat_cli_config_path")
...
def get_default_config_dir():
    ...
    if os.name == 'nt':
        ...
        if appdata:
            return os.path.join(appdata, "animeiat_cli")
        else:
            return os.path.join(home, ".config", "animeiat_cli")
    else:
        return os.path.join(home, ".config", "animeiat_cli")
...
def get_config_dir():
    ...
    if not os.path.exists(path):
        _migrate_config_dir(path)
    os.makedirs(path, exist_ok=True)
    return path


def _migrate_config_dir(new_dir):
    _OLD_CONFIG_DIRS = ["pyanime", "byanime"]
    home = os.path.expanduser("~")
    for old_name in _OLD_CONFIG_DIRS:
        if os.name == 'nt':
            appdata = os.environ.get("APPDATA")
            old_dir = os.path.join(appdata, old_name) if appdata else os.path.join(home, ".config", old_name)
        else:
            old_dir = os.path.join(home, ".config", old_name)
        ...
        for root, dirs, files in os.walk(old_dir):
            for fname in files:
                old_f = os.path.join(root, fname)
                rel = os.path.relpath(old_f, old_dir)
                new_f = os.path.join(new_dir, rel)
                if not os.path.exists(new_f):
                    ok = False
                    break
                old_hash = _hash_file(old_f)
                new_hash = _hash_file(new_f)
...
def save_config(cfg):
    ...
    try:
        ...
        print(f"[animeiat-cli] Warning: could not save config: {e}")

"""
Database infrastructure for animeiat-cli — favorites, watch history, accounts, progress.
"""
...
def get_db_path():
    return os.path.join(get_config_dir(), "animeiat_cli.db")
...
def _migrate_old_db():
    ...
    if os.path.exists(new_db):
        return
    old_names = ["pyanime.db", "byanime.db"]
    for old_name in old_names:
        old_db = os.path.join(config_dir, old_name)
        if os.path.exists(old_db):
            try:
                ...
                print(f"[animeiat-cli] Warning: DB migration copy failed: {e}")
                try:
                    os.remove(new_db)
...
def refresh_mal_token(client_id, refresh_token):
    ...
    try:
        ...
        print(f"[animeiat-cli] Warning: refresh_mal_token failed: {e}")
    return None
...
def fetch_anime_metadata(search_term):
    ...
    try:
        ...
        print(f"[animeiat-cli] Warning: fetch_anime_metadata failed: {e}")
        _metadata_cache[key] = None
        return None
...
def fetch_anilist_user_list(token, status_filter=None):
    ...
    try:
        ...
        print(f"[animeiat-cli] Warning: fetch_anilist_user_list viewer query failed: {e}")
        return []
    ...
    try:
        ...
        print(f"[animeiat-cli] Warning: fetch_anilist_user_list list query failed: {e}")
        return []
...
def fetch_mal_user_list(token, status_filter=None):
    ...
    try:
        ...
        print(f"[animeiat-cli] Warning: fetch_mal_user_list failed: {e}")
        return []
